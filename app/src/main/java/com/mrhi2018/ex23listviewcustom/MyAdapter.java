package com.mrhi2018.ex23listviewcustom;
...
public class MyAdapter extends BaseAdapter {
    ...
    //이 메소드가 항목(Item)하나의 View객체를 만들어서 리턴하는 작업수행
    @Override
    public View getView(int position, View convertView, ViewGroup parent) {
        //첫번재 매개변수(파라미터)로 전달되어 온 position번째의 Data로
        //View객체를 만들어서 리턴시켜줘야 함.

        //1. new View(뷰객체를 만드는 작업)
        //혹시 재활용할 View가 없는가?? 없다면 만들고 ... 있다면 그냥 데이터만 연결..
        // 두 번째 파라미터가 재활용할 뷰 참조변수임
        if( convertView==null ){
            //res폴더안에 layout폴더안에 listview_item.xml의 모양으로
            //View객체를 생성해주는 능력을 가진 객체에게 의뢰..
